    ...
    def update_map(self, depth, current_pose, semantic, goal_category_id):
        ...
        for di in range(-k, k + 1):
            for dj in range(-k, k + 1):
                self.geocentric_flat[np.clip(i + di, 0, self.geocentric_flat.shape[0] - 1),
                                     np.clip(j + dj, 0, self.geocentric_flat.shape[1] - 1),
                                     1] = 1
        ...
        for index, depth in enumerate(depth_semantic[:1]):
            ...
            """
            if len(i) > 0:
                #print('i, j:', list(zip(i, j)))
                print('Current pose:', x_cell, y_cell)
                print('Geocentric flat min distance:', dst.min())
                print('Number of pixels with dist < 1m:', (dst < 100 / self.resolution).sum())
            """
            ...
            """
            if semantic.max() > 0:
                print('Semantic max:', semantic.max())
                print('Min depth of goal:', depth[semantic > 0].min())
                print('Max depth of goal:', depth[semantic > 0].max())
                print('Geocentric flat max:', semantic_geocentric_flat[:, :, 1].max())
            """
